...
typedef enum
{
  ...
  ST_COMM_START     , // на зымыкание ”станавливаю состо€ние START на размыкание выключаем реле cnc0
  ST_COMM_STOP      ,
...
typedef struct
{
  ...
  uint16_t bTestCurr2   : 1; // Ќачать провер€ть рост тока дуги
  uint16_t bIgnitionOk  : 1; // ѕоджиг сработал - выдать на „ѕ” сигнал готовоность
...
typedef  enum
{
  ...
  EV_COMM_START     , // „ѕ” -> START -> состо€ние —“ј–“. Ћеха, камандует ѕќƒ∆»√, ожидаем тока 20мс, выключаем поджиг
  EV_METAL_CONTACT  ,
  EV_COMM_FIRE      ,
