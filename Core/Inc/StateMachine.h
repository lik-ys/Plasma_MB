...
typedef enum
{
  FIRE_START        ,
  PILOT_ARC         ,
  CHOPPER_START     , // синхронное выключение ячеек от команды  bit0 в  REG_W_CNTRL_START
  FIRE_IN_START     , // TODO
...
{  
  ...
  struct {
    ...
    uint16_t bChillerFail_t : 1;    // 6 – ошибка чиллера    
       
    uint16_t bStartCNC      : 1;    // 7 - 1 - Start, 0 - Stop CNC
    uint16_t bFireIn        : 1;    // todo внешний поджиг    
    uint16_t bChopperStart  : 1;    // 7 - синхронное выключение ячеек от команды  bit0 в  REG_W_CNTRL_START    
  }bit;   
}RgCntrl_t;
