...
typedef union STATUS
{
  ...
  struct
  {
    uint16_t bPwr          : 1;    // 0 бит-Вкл/выкл источника.
    uint16_t bArc          : 1;    // 1 бит- Вкл/выкл дежурной дуги.
    uint16_t bFire         : 1;    // 2 бит- Вкл/выкл поджига.
    ...
    uint16_t bEepromWr     : 1;     // 15 бит - Запись в EEpprom
  }bit;
}StatProc_t;
...
typedef union STATUS_1
{
  ...
  struct
  {
    uint16_t bReserv    : 1; // 0 бит- не используется
    uint16_t bCell_1    : 1; // 1 бит- Вкл/выкл ячейки 1.
    uint16_t bCell_2    : 1; // 2 бит- Вкл/выкл ячейки 2.
    uint16_t bCell_3    : 1; // 3 бит- Вкл/выкл ячейки 3.
    uint16_t bCell_4    : 1; // 4 бит- Вкл/выкл ячейки 4.
    uint16_t bCell_5    : 1; // 5 бит- Вкл/выкл ячейки 5.
    uint16_t bCell_6    : 1; // 6 бит- Вкл/выкл ячейки 6.
    uint16_t bReserv1   : 1; // 7 бит- не используется     
  }bit;
} StatCell_t;
